...
public class AccommodationServiceImpl implements AccommodationService{
    ...
    public AccommodationServiceImpl(AccommodationRepo accommodationRepo, BookingRepo bookingRepo) {
...
@Override
    public Response<AccommodationDTO> getAvailableRoomsByDateAndType(LocalDate checkInDate, LocalDate checkOutDate, String roomType) {
        Response<AccommodationDTO> response = new Response<>();

        try {
            List<Booking> bookings = bookingRepo.findByCheckInDateBetweenOrCheckOutDateBetween(checkInDate, checkOutDate);
            List<String> bookedRoomsId = bookings.stream().map(booking -> booking.getAccommodation().getId()).toList();

            List<Accommodation> availableRooms = accommodationRepo.findByRoomTypeLikeAndIdNotIn(roomType, bookedRoomsId);
